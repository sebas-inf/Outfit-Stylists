...
const CreatePost = ({user}) => {
  ...
  const [displayInvalidFieldsW, setDisplayInvalidFieldsW] = useState(false);
  const [flare, setFlare] = useState('other');
        ...
        <div className='flex lg:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full'>
          ...
          <div className='bg-secondaryColor p-3 flex flex-0.7 w-full'>
            ...
            <div className='flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-420'>
              {loading && <Spinner/>}
              {wrongImageType && <p>Wrong image type. Supported image types(.JPG, .SVG, .PNG, .GIF)</p>}
              { !imageAsset ? (
              <label>
                <div className='flex flex-col items-center justify-center h-full cursor-pointer'>
                  ...
                  <p className='mt-32 mx-1 text-center text-gray-400'>Use high-quality JPG, SVG, PNG, or GIF less than 20 MB</p>
                </div>
                <input
                  type="file"
                  name="upload-image"
              ...
              <div className='flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full'>
                ...
                <div>
                  <p className='mb-2 mx-1 font-semibold text-lg sm:text-xl'>Choose Post Flare's</p>
                  <select
                    ...
                    value={flare}
                  >
                    <option value="other" className='bg-white'>Select Flare's for this post</option>
                    { flares.map(({ name }) => (
