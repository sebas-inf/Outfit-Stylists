...
function renderDocument(options) {
  return new Promise((resolve2, reject) => {
    if (!useThreads) {
      resolve2(getDocumentHtml(options.studioRootPath, options.props, options.importMap, options.isCoreApp));
      return;
    }
...
function renderDocumentFromWorkerData() {
  ...
  const {
    ...
    jsx: "automatic",
    loader: "jsx"
  }), html = getDocumentHtml(studioRootPath, props, importMap, isCoreApp);
  node_worker_threads.parentPort.postMessage({
    type: "result",
    html
  }), unregister(), unregisterJs();
}
function getDocumentHtml(studioRootPath, props, importMap, isCoreApp) {
  const Document = getDocumentComponent(studioRootPath, isCoreApp), css = props?.css?.map((url) => {
    try {
...
function getDocumentComponent(studioRootPath, isCoreApp) {
  debug$2("Loading default document component from `sanity` module");
  const {
    BasicDocument
  } = require("sanity"), {
    DefaultDocument
  } = require("sanity"), Document = isCoreApp ? BasicDocument : DefaultDocument;
...
function getEntryModule(options) {
  ...
  return isCoreApp ? coreAppEntryModule.replace(/%APP_LOCATION%/, JSON.stringify(appLocation || "./src/App")) : (relativeConfigLocation ? entryModule : noConfigEntryModule).replace(/%STUDIO_REACT_STRICT_MODE%/, JSON.stringify(!!reactStrictMode)).replace(/%STUDIO_CONFIG_LOCATION%/, JSON.stringify(relativeConfigLocation)).replace(/%STUDIO_BASE_PATH%/, JSON.stringify(basePath || "/"));
}
