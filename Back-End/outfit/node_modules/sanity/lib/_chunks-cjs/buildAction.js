...
async function buildSanityStudio(args, context, overrides) {
  const timer = timing.getTimer(), {
    ...
    "source-maps": !1,
    ...args.extOptions
  }, unattendedMode = !!(flags.yes || flags.y), defaultOutputDir = path__default.default.resolve(path__default.default.join(workDir, "dist")), outputDir = path__default.default.resolve(args.argsWithoutOptions[0] || defaultOutputDir), isCoreApp = cliConfig && "__experimental_coreAppConfiguration" in cliConfig;
...
Including the following environment variables as part of the JavaScript bundle:`), envVarKeys.forEach((key) => output.print(`- ${key}`)), output.print(""));
  ...
  try {
    ...
    const bundle = await previewServer.buildStaticFiles({
      ...
      appLocation: cliConfig && "__experimental_coreAppConfiguration" in cliConfig ? cliConfig.__experimental_coreAppConfiguration?.appLocation : void 0,
      isCoreApp
    });
    trace.log({
    ...
    spin.text = `Build Sanity ${isCoreApp ? "application" : "Studio"} (${buildDuration.toFixed()}ms)`, spin.succeed(), trace.complete(), flags.stats && (output.print(`
