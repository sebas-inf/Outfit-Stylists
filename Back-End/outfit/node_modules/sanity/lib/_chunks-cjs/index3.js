...
function StructureError(t0) {
  ...
  const {
    ...
    t
  } = sanity.useTranslation(structure.structureLocaleNamespace), stack = cause?.stack || error.stack, showStack = stack && !(cause instanceof structure.SerializeError) && !error.message.includes("Module build failed:");
  let t1;
  $[0] !== cause ? (t1 = cause instanceof structure.SerializeError ? cause.path : [], $[0] = cause, $[1] = t1) : t1 = $[1];
  const path = t1, helpId = cause instanceof structure.SerializeError && cause.helpId || error.helpId, handleReload = _temp$3;
  let t2;
...
const DocumentTitle = (props) => {
  const $ = reactCompilerRuntime.c(7), {
    ...
    t
  } = sanity.useTranslation(structure.structureLocaleNamespace), isNewDocument = !editState?.published && !editState?.draft, documentValue = editState?.version || editState?.draft || editState?.published, schemaType = schema.get(documentType), {
    value,
...
`, isSaveHotkey = isHotkeyEsm.isHotkey("mod+s"), StructureTool = React.memo(function(t0) {
  ...
  let t13;
  $[17] !== handleRootCollapse || $[18] !== handleRootExpand || $[19] !== t10 || $[20] !== t11 || $[21] !== t12 || $[22] !== t9 ? (t13 = /* @__PURE__ */ jsxRuntime.jsxs(StyledPaneLayout, { flex: 1, height: t9, minWidth: t10, onCollapse: handleRootCollapse, onExpand: handleRootExpand, children: [
    t11,
    t12
...
function StructureToolBoundary(t0) {
  ...
  let t9;
  $[8] !== defaultDocumentNode || $[9] !== structure$1 ? (t9 = /* @__PURE__ */ jsxRuntime.jsxs(structure.StructureToolProvider, { defaultDocumentNode, structure: structure$1, children: [
    t7,
    t8
