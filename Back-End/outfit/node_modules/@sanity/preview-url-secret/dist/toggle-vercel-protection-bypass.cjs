...
async function enableVercelProtectionBypass(_client, secret) {
  const client = _client.withConfig({ apiVersion: constants.apiVersion }), patch = client.patch(constants.vercelProtectionBypassSchemaId).set({ secret });
...
async function disableVercelProtectionBypass(_client) {
  const client = _client.withConfig({ apiVersion: constants.apiVersion }), patch = client.patch(constants.vercelProtectionBypassSchemaId).set({ secret: null });
  await client.transaction().createIfNotExists({ _id: constants.vercelProtectionBypassSchemaId, _type: constants.vercelProtectionBypassSchemaType }).patch(patch).commit({ tag: constants.tag });
}
